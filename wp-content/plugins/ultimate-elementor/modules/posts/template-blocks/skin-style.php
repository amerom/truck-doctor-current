...
abstract class Skin_Style {
				...
				?>
					...
					<?php if ( ! empty( $icon ) || ! empty( $new_icon ) ) { ?>
						...
						if ( $author_migrated || $author_icon_is_empty ) {
							\Elementor\Icons_Manager::render_icon( $new_icon, [ 'aria-hidden' => 'true' ] );
						} elseif ( ! empty( $icon ) ) {
							?>
		...
		if ( 'yes' === $this->get_instance_value( 'show_cta' ) ) {
			...
			?>
				<a <?php echo $this->get_render_attribute_string( 'cta_link' . get_the_ID() ); ?>>
					...
					<span><?php echo apply_filters( 'uael_post_cta_text', $this->get_instance_value( 'cta_text' ), get_the_ID(), $settings ); ?></span>
				</a>
			<?php
	...
	public function render_pagination() {
		...
		if ( ! in_array( $this->get_instance_value( 'post_structure' ), [ 'normal', 'featured', 'masonry' ], true ) ) {
			return;
		}
	...
	public function get_slider_attr() {
		...
		$show_dots   = ( in_array( $this->get_instance_value( 'navigation' ), [ 'dots', 'both' ], true ) );
		$show_arrows = ( in_array( $this->get_instance_value( 'navigation' ), [ 'arrows', 'both' ], true ) );
	...
	public function render_filters() {
		...
		if ( ! in_array( $this->get_instance_value( 'post_structure' ), [ 'masonry', 'normal' ], true ) ) {
			return;
		}
