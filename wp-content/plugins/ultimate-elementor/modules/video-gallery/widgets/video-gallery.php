		...
		if ( parent::is_internal_links() ) {
			...
			$this->add_control(
				...
				[
					'type'            => Controls_Manager::RAW_HTML,
					/* translators: %1$s doc link */
					'raw'             => sprintf( __( '%1$s Get Wistia Link and Thumbnail text » %2$s', 'uael' ), '<a href="https://uaelementor.com/docs/video-gallery-widget/#for-wistia-video-" target="_blank" rel="noopener">', '</a>' ),
					'content_classes' => 'uael-editor-doc',
				]
			);
		...
		if ( 'icon' === $settings['play_source'] ) {
			...
			if ( UAEL_Helper::is_elementor_updated() ) {
				if ( ! empty( $settings['play_icon'] ) || ! empty( $settings['new_play_icon'] ) ) :
					...
					?>
					<span class="<?php echo 'elementor-animation-' . $settings['hover_animation']; ?> uael-vg__play-icon">
						<?php
						if ( $is_new || $migrated ) {
