		...
		if ( parent::is_internal_links() ) {
			$this->add_control(
				...
				[
					...
					'raw'             => sprintf( __( 'Click %1$s here %2$s to learn more about this.', 'uael' ), '<a href="https://uaelementor.com/docs/image-gallery-widget/#advanced-lightbox-actions/" target="_blank" rel="noopener">', '</a>' ),
					'content_classes' => 'uael-editor-doc',
				]
			);
		}
		...
		if ( parent::is_internal_links() ) {
			...
			$this->add_control(
				...
				[
					'type'            => Controls_Manager::RAW_HTML,
					/* translators: %1$s doc link */
					'raw'             => sprintf( __( '%1$s Display Caption below image in the lightbox » %2$s', 'uael' ), '<a href="https://uaelementor.com/docs/image-gallery-widget/#caption-below-image-of-lightbox" target="_blank" rel="noopener">', '</a>' ),
					'content_classes' => 'uael-editor-doc',
				]
			);
			...
			if ( UAEL_Helper::is_elementor_updated() ) {
				...
				if ( $has_icon ) {
					...
					if ( $is_new || $migrated ) {
						...
						ob_start();
						\Elementor\Icons_Manager::render_icon( $settings['new_overlay_icon_hover'], [ 'aria-hidden' => 'true' ] );
						$output .= ob_get_clean();
